...
class AttentionHead(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        ...
        # Aplicar máscara causal si existe, llenando con -inf donde se enmascara
        if mask is not None:
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.d_k = self.d_q = self.d_v = d_model // num_attention_heads

        # Crear una lista de AttentionHeads
        self.heads = nn.ModuleList(
            [
                AttentionHead(d_model, self.d_k, self.d_q, self.d_v)
    ...
    def forward(self, hidden_state, mask=None):
        ...
        # Concatenar por la última dimensión (d_v * num_heads = d_model)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        # Feed-forward con residual + layer norm
